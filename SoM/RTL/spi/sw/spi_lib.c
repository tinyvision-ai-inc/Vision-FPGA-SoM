...
/*
int spi_send(uint8_t cmd, uint8_t val[4], uint8_t *status)
{
...
int spi_get_id_status(uint8_t *id, uint8_t *status)
{
   uint8_t command[] = {0x05, 0x00, 0x00, 0x00}; //nop
   uint8_t status_recv = 0;
   uint8_t id_recv = 0;
   uint32_t retries = 0;
   uint32_t max_retries = 1000;
      ...
      xfer_spi(command, 1);
      status_recv = command[0];
      id_recv = command[1];
      retries++;
   printf("%x %x %x %x", command[0], command[1], command[2], command[3] );
   if(status != NULL){
      *status = status_recv;
   }
